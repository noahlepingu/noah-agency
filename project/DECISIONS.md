...
| 2026-09-16 | **Phase 7 Final Review : verdict A LIVRER AVEC RESERVES** | Final Product Reviewer | Revue finale du systeme et du build exemple (16 pages). Verdict : base de production saine et pipeline fonctionnel, mais 11 corrections requises avant 1er client reel. Bloquants Gate 4 : C-01 (astro 5.18.2, npm audit 3 vuln dont 1 critical, upgrade >= 7.3.2), C-04 (pas de public/_headers), C-06 (pas de lien "Gerer les cookies" dans Footer), C-07 (pas de security.txt), C-08 (pas de Dependabot), C-09 (pas de npm audit dans CI), C-05 (sous-traitant formulaire a decider). Qualite Gate 3 : Q1 BUG-QA-02 "cuisine cuisine" NON corrige (FR + EN), Q2 contraste hours gray-400 2.54:1 / success 3.77:1 NON corrige (WCAG 1.4.3), Q3 404/500 bundle CSS sans .ds-btn (D-PERF-02), Q4 JSON-LD Restaurant jamais emis (code mort schema.js, D-CS-03). Rapport complet : `project/docs/FINAL_REVIEW.md`. |
...
| D-FE-RE-01 | **Header transparent—solide : themes sombre/clair par variante hero** | Transparent sur hero, `ds-header--solid` au scroll > 40px ou page sans hero (404/500). Sur hero image pleine (variante A) et hero typographique (variante C), le header passe en `ds-header--dark` (logo/nav creme) via `[data-hero-dark]` ; variante split (B) = encre. Ecart DA documente : la DA prevoyait le header sombre sur TOUTES les variantes — impossible WCAG 1.4.3 pour la variante B (texte encre sur surface claire). Nav/footer i18n via `getPageTexts(data, lang, 'common')`. | OUI (Gate 2) |
| D-FE-RE-02 | **Theme genere : variantes explicites de marque + defauts DA 2.6** | `generate-site.mjs` accepte `primary_light/dark`, `secondary_light/dark`, `accent_light/dark` et `on_*` explicites dans `branding:` (utilises tels quels — fidelite DA), sinon calcule (darkTokenPair WCAG). Fallbacks par defaut = palette DA 2.6 (`#3A2E27`/`#A67C52`/`#B4542C`, fini `#B91C1C`/`#F59E0B`/`#DC2626`). `contrast-check.mjs` aligne (fallbacks DA). Exemple : client_data.yaml porte l'integralite des tokens DA 2.7 + `fonts.heading: Fraunces`. | — |
| D-FE-RE-03 | **Formulaire de reservation : prop `card` (mode carte DA §5.7)** | `ReservationForm` gagne `card?: boolean` : rendu sans `ds-section` ni en-tete, dans une `.ds-form-card` (surface-raised radius 16 padding 32→40) posee dans une grille `.ds-layout-form` 7fr/5fr (>= 1024px) avec colonne `.ds-form-side` (OpeningHours + adresse/tel/email + note « >8 personnes »). Même grille sur `/contact` (formulaire + infos, puis section carte standalone). Styles formulaires deplacés de ContactForm scope → `utilities.css` (bug preexistant : ReservationForm non style sur sa page). | OUI (Gate 2) |
| D-FE-RE-04 | **Navigation + CTA mobile reellement fonctionnels** | Burger = menu offcanvas local (focus-trap, Escape, aria-expanded, `hidden`) — remplace l'ancre page de la DA ; la barre sticky « Reserver » + tel est parcourue `data.phone || data.footer.phone || data.contact_form.phone` (base passait `data.components` sans `phone` top-level : barre jamais rendue — corrige). | — |
| D-FE-RE-05 | **Fraunces ajoutee a l'enum `FONTS` de validation** | `validation-core.mjs` : `FONTS = ['Inter', 'Playfair Display', 'DM Sans', 'Libre Baskerville', 'Fraunces']`. Necessaire : la DA (D-UX-16) remplace Playfair par Fraunces dans le template — sans cet ajout, `fonts.heading: Fraunces` est refuse en validation bloquante. Effet uniquement sur le template restaurant (les autres templates gardent Playfair). | — |

## Decisions Refonte Visuelle — Direction artistique « Bistrot lyonnais » (2026-09-16)

Statut : **PROPOSEES par l'UX Designer — VALIDEES par le Tech Lead — a
confirmer par Noah en Gate 2** (refonte esthetique du template flagship
restaurant, sans changement de perimetre fonctionnel).
Reference : `project/design/UX_REDESIGN_RESTAURANT.md` (livrable complet de
la direction artistique). Contexte : Noah juge le site « trop generique,
impression de template generee par IA » — reference d'ambiance analysee
(sans copie) : Bistrot & Co, Lyon.

| ID | Decision | Detail | A valider par Noah ? |
| -- | -------- | ------ | --------------------- |
| D-UX-15 | **Palette par defaut « creme / brun sombre / terracotta »** | Surfaces `#FBF7F0` (fond), `#FFFDF9` (raised), `#F3EBDF` (alt), `#2B2320` (dark) ; textes `#2A211C` / `#5C5148` ; accent terracotta `#B4542C` (+ dark `#8F3E1F` pour textes/liens 6.84:1, light `#F2DCCF`) ; lignes `#E4D9C8` / `#9A7B5E`. Les gris froids systeme restent disponibles en tant que tokens fondationnels mais ne sont plus utilises comme fonds/textes du template restaurant. Bloc `@layer ds-brand` complet fourni (UX_REDESIGN §2.6). | OUI (Gate 2) |
| D-UX-16 | **Typo titres = Fraunces (woff2 local), Inter conserve en corps** | Fraunces variable 400/500/600/700 embarque localement (ADR-010, jamais Google Fonts CDN) ; Playfair Display retire des titres. Regles : H1 = 400-500 (jamais 700), H2/H3 = 500, gras 700 reserve aux boutons/badges ; nouvelle echelle `--text-6xl: 64px` (H1 hero), `--text-4xl: 40px` (H2), `--text-2xl: 26px` (H3), `--text-7xl: 120px` (404/monogramme) ; prix en 600 `tabular-nums` ; chapeau editorial Fraunces italic 1/par page. | OUI (Gate 2) |
| D-UX-17 | **Rythme de sections : respirer par sections, pas par padding** | Nouveaux tokens `--space-28: 112px`, `--space-32: 128px`, `--space-40: 160px` ; sections >= 96px desktop (64/72/96 mobile/tablet/desktop), hero 88-92svh ; `--container-max: 1160px`, `--content-max: 680px` ; alternance obligatoire des fonds (surface / surface-alt), jamais 3 paves identiques d'affilee ; grille de 8 au-dela de 24px. | OUI (Gate 2) |
| D-UX-18 | **Anti-template : menu = carte typographique, galerie asymetrique, avis = citations, footer sombre chaud** | Menu : 0 card — nom (Fraunces 26/500) + ligne de points + prix aligne a droite (Inter 600 tabular-nums), indicateurs `(v)/(p)/(bio)` 12px accent-dark ; galerie : grille 6 colonnes asymetrique (grande 4/6 + portrait 2/6 + 2 carres + panoramique 6/6), 0 ombre, angles vifs ; avis : blockquote sans fond/bordure/ombre ; footer : `--color-surface-dark` (jamais noir pur `#111827`), 4 colonnes, bas de page border-top + « Site fait a Lyon ». Cards restreintes aux formulaires/badges (radius 16, 0 ombre). | OUI (Gate 2) |
| D-UX-19 | **Etats speciaux narratifs + header transparent→solide + CTA mobile sticky** | 404 « Cette page est partie gouter ailleurs » / 500 « Une erreur de cuisine » (fond surface-alt, monogramme en filigrane) ; cookie banner surface-dark radius 12 ; header transparent sur hero puis solide au scroll (>40px, sans ombre — ligne de separation `--color-line`) ; barre sticky mobile « Reserver » + tel (<768px, pattern de conversion restaurant) ; boutons radius 4px partout (12px = template IA). | OUI (Gate 2) |
| D-UX-20 | **Hero : 3 variantes, jamais gris centre** | A — pleine hauteur 92svh, image cover cadree centre-bas, voile 12% (PAS de gradient) ; B — split editorial texte/image sans radius ; C — typographique sombre sans image (cas par defaut du systeme : hero_image null — monogramme Fraunces 200 en filigrane, titre creme, 1 CTA accent + 1 lien texte). Regle commune : 1 H1 (max 3 mots/ligne) + 1 chapeau + 1 CTA principal + 1 lien texte max. | OUI (Gate 2) |
| D-UX-21 | **Images de demo : 10 Picsum (IDs verifies) a remplacer par les photos client en Gate 2** | URLs et usages documentes (UX_REDESIGN §6) : hero 1920x1080, ambiance 1200x900, plat 1200x900, galerie 6 formats (grande/portrait/carres/panoramique), a-propos 3:2. Regles : alt descriptif obligatoire, commentaire « image de demo » dans le template (pas dans le alt), width/height explicites, lazy+async hors 1er ecran, object-fit cover. Les vrais noms/visages des temoignages ne sont jamais inventes (a fournir par le client). | OUI (Gate 2) |

## Decisions QA — Verification de la refonte visuelle (2026-09-18)

Statut : **QA — Validation technique de la refonte « Bistrot lyonnais » (commit
be42a43)**. Verdict : **PEUT-ETRE-LIVRE** (aucun bug majeur, reserves cote
localisation EN documentees). Reference : `project/tests/TEST_RESULTS_REDESIGN.md`.

| ID | Decision | Detail |
| -- | -------- | ------ |
| D-QA-RE-01 | **Refonte validee sans intervention code — 5 bugs QA-02→06 corriges, 2 partiellement (01/08)** | Regression completes validees : body EN traduit (QA-01), « cuisine cuisine » (QA-02), nav EN (QA-03), patterns seo_title (QA-04), meta desc ≠ title (QA-05), 0 script cookie sans tiers (QA-06). QA-01/QA-08 partiellement corriges : reliquats categorises en nouveaux bugs QA-10/11/12 (metadata SEO EN, JSON-LD, footer) — tous cotes EN, pages FR non impactees. QA-07 (monotype) inchange : hors perimetre refonte. |
| D-QA-RE-02 | **BUG-QA-09 (lien /en/menu 404) = reserve pre-livraison multilingue** | Bug Mineur cote EN, cause identifiee et correctif 1 ligne (Menu.astro : `localizePath` au lieu du prefixe `/en/` inconditionnel). A corriger avant toute livraison commerciale multilingue ; non bloquant pour la demarche FR / demo countdown. |
| D-QA-RE-03 | **Pages legales noindex/nofollow conservees, 404/500 narratives OK** | Conformite SEO conservee : mentions-legales/confidentialite/plan-du-site en noindex ; 404 « Cette page est partie gouter ailleurs » + CTA en .ds-btn (correctif FINAL_REVIEW Q3 toujours en place) ; og:image sur toutes les pages (Q6). |
| D-QA-RE-04 | **H2 intro absent = observation contenu (QA-13), pas une regression code** | `intro_subtitle` non fourni dans le YAML exemple (champ optionnel) : le composant rend correctement l'eyebrow + chapeau. A completer cote contenu client (SIREN/coordonnees du type), pas cote frontend. |
| D-QA-RE-05 | **Recommandation : traiter QA-09/10/11/12 dans la passe Correction Phase 6, QA-13 dans la passe Contenu** | Priorisation : QA-09 (404) avant toute vente EN ; QA-10/11 (metadata locales SEO) impactent le SEO local EN ; QA-12 (footer) cosmétique ; QA-13 (contenu) a alimenter avec le brief client reel. Aucune action requise d'un autre domaine sans coordination. |
