...
Mis a jour a chaque tache livree. Derniere mise a jour : 2026-09-16 (Phase 7 — Sprint Gate 4 Infrastructure securite : C-04 headers HTTP, C-07 security.txt, C-08 dependabot, C-09 npm audit CI + contrast CI — livrees et verifiees ; restent C-01 (upgrade Astro en cours par le sprint frontend) et C-06 (frontend) avant Gate 4).
...
| 2026-09-16 | Phase 7 — Sprint Gate 4 : Infrastructure securite (C-04, C-07, C-08, C-09) | DevOps | FAIT (ACTE) | **4 conditions bloquantes Gate 4 levees** : C-04 `_headers` Cloudflare Pages (HSTS, nosniff, DENY, Referrer-Policy, Permissions-Policy, CSP stricte justifiee par grep build) cree dans `templates/restaurant/public/` (canonique, copie dans chaque build) + miroir `public/` ; C-07 `security.txt` RFC 9116 avec placeholders documentes (contact/canonical/expires/policy a completer par Noah) ; C-08 `.github/dependabot.yml` (npm weekly, 3 PR max, reviewer Noah placeholder) ; C-09 `npm audit --audit-level=high` bloquant + bonus `npm run contrast` dans `ci.yml` (Node 22 — dependance sprint Astro 7). Verifications : build 16 pages OK, `dist/exemple-restaurant/_headers` + `.well-known/security.txt` presents, 39 blocs script inline / 32 styles inline / 4 iframes OSM grepes pour justifier la CSP, npm audit 0 vuln, 30/30 tests, contrast all pass. Docs : DEPLOYMENT §12, TODO_PRODUCTION (headers/security.txt/formulaire-CSP), CI_CD, MAINTENANCE_PLAN (renewal security.txt), FINAL_REVIEW « Sprint Gate 4 ». Decisions D-DEVOPS-10 a 15. **Dependance signalee au sprint frontend : deploy-site.yml encore en node-version 20 — cassera avec astro@7.3.2.** |
